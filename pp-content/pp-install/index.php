...
if (isset($_POST['test_databse_request'])) {
    ...
    try {
        ...
        // Use uploaded SQL backup if provided, otherwise the bundled schema
        if (isset($_FILES['dbFile']) && $_FILES['dbFile']['error'] === UPLOAD_ERR_OK) {
            $dbFileExt = strtolower(pathinfo($_FILES['dbFile']['name'], PATHINFO_EXTENSION));
            if ($dbFileExt !== 'sql') {
                throw new Exception("Only SQL files are allowed for manual restore.");
            }

            $sqlContent = file_get_contents($_FILES['dbFile']['tmp_name']);
        } else {
    ...
    <div class="container p-2 p-sm-4">
        ...
        <div class="col-lg-5 mx-auto all-pages">
            ...
            <div class="card" id="page2">
                ...
                <div class="card-body">
                    <form class="database-config-extra" id="dbForm" enctype="multipart/form-data">
                        <div class="row gy-3">
                            <!-- Database Driver -->
                            <div class="col-12">
                                        ...
                                        <div class="col-6">
                                            <div class="form-control-wrap p-2 border rounded" style="height: 40px;">
                                                <div class="form-check form-check-inline" style="margin-top: -2px;">
                                                    <input class="form-check-input" type="checkbox" id="driverMysql"
                                                        name="dbDriver" value="mysql" checked>
                                                    <label class="form-check-label" for="driverMysql">MySQL /
                                                        MariaDB</label>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <div class="form-control-wrap">
                                        <input type="text" class="form-control" id="tablePrefix" placeholder="pp_"
                                            value="pp_">
                                    </div>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="dbFile" class="form-label">Restore From SQL Backup (Optional)</label>
                                    <div class="form-control-wrap">
                                        <input type="file" class="form-control" id="dbFile" name="dbFile"
                                            accept=".sql">
                                    </div>
                                    <small class="text-muted">Upload a database backup SQL file to restore data during installation. Leave empty to use the default schema.</small>
                                </div>
                            </div>
                            <div class="col-12">
                                <button type="button" class="btn btn-outline-primary w-100 test-connection-btn"
                                    id="btnTestConnection">Check & Import</button>
                            </div>
                        </div>
                    </form>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-control-wrap">
                                        ...
                                        <div class="password-strength mt-1">
                                            <small class="text-muted">Password strength: <strong><span
                                                        id="passwordStrength">None</span></strong></small>
                                            <div class="strength-meter mt-2">
                                                <div class="strength-fill" id="passwordStrengthMeter"
                                                    style="height: 2px; border-radius: 5px;"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="card-body text-center mt-2">
                    <div class="m-2">
                        <svg xmlns="http://www.w3.org/2000/svg" style="width: 50px; height: 50px;" viewBox="0 0 24 24"
                            fill="none" stroke="#5f38f9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="alert alert-warning mb-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1 ms-2">
                                ...
                                <p class="mb-0">For security reasons, please delete or rename the
                                    <code>pp-install</code> directory after installation.</p>
                            </div>
                        </div>
                    </div>
    ...
    <script data-cfasync="false">
        ...
        $(document).ready(function () {
            ...
            $('#btnTestConnection').on('click', function () {
                ...
                formData.append('test_databse_request', 'true');
                formData.append('dbDriver', driver);
                formData.append('dbHost', $('#dbHost').val());
                formData.append('dbPort', $('#dbPort').val());
                formData.append('dbName', $('#dbName').val());
                formData.append('dbUsername', $('#dbUsername').val());
                formData.append('dbPassword', $('#dbPassword').val());
                formData.append('tablePrefix', $('#tablePrefix').val());

                let dbFileInput = document.getElementById('dbFile');
                if (dbFileInput && dbFileInput.files.length > 0) {
