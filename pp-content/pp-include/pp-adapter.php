...
<?php
    ...
    if(isset($_POST['action'])){
        ...
        if($action == ""){
            ...
            if($action == "system-settings-backup-create"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        echo json_encode(['status' => "false", 'title' => 'Demo Restriction', 'message' => 'This feature is disabled in the demo version.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        ...
                        try {
                            backupDatabasePDO($backupDir . $filename);

                            echo json_encode(['status' => 'true', 'title' => 'Backup Created', 'message' => 'Database backup created successfully.', 'filename' => $filename, 'csrf_token' => $new_csrf_token]);
                        } catch (Throwable $e) {
                            echo json_encode(['status' => 'false', 'title' => 'Backup Failed', 'message' => $e->getMessage(), 'csrf_token' => $new_csrf_token]);
                        }
                    }
                }else{
                    echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Invalid request' , 'csrf_token' => $new_csrf_token]);
            ...
            if($action == "system-settings-backup-restore"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        echo json_encode(['status' => "false", 'title' => 'Demo Restriction', 'message' => 'This feature is disabled in the demo version.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        ...
                        if (!move_uploaded_file($uploadedFile['tmp_name'], $backupDir . $filename)) {
                            echo json_encode([
                                'status' => 'false',
                                'title' => 'Upload Failed',
                                'message' => 'Failed to move uploaded file.',
                        ...
                        try {
                            restoreDatabasePDO($backupDir . $filename);

                            echo json_encode(['status' => 'true', 'title' => 'Restore Successful', 'message' => 'Database has been restored from the backup file.', 'csrf_token' => $new_csrf_token]);
                        } catch (Throwable $e) {
                            echo json_encode(['status' => 'false', 'title' => 'Restore Failed', 'message' => $e->getMessage(), 'csrf_token' => $new_csrf_token]);
                        }
                    }
                }else{
                    echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Invalid request' , 'csrf_token' => $new_csrf_token]);
            ...
            if($action == "system-settings-backup-delete"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        echo json_encode(['status' => "false", 'title' => 'Demo Restriction', 'message' => 'This feature is disabled in the demo version.', 'csrf_token' => $new_csrf_token]);
                    }else{
                        ...
                        $filename = basename(escape_string($_POST['backup_file'] ?? ''));

                        if ($filename == "" || !is_file($backupDir . $filename)) {
                            echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Backup file not found.' , 'csrf_token' => $new_csrf_token]);
                            exit();
                        }

                        if (unlink($backupDir . $filename)) {
                            echo json_encode(['status' => 'true', 'title' => 'Deletion Successful', 'message' => 'Backup file has been deleted.', 'csrf_token' => $new_csrf_token]);
                        } else {
                            echo json_encode(['status' => 'false', 'title' => 'Deletion Failed', 'message' => 'Unable to delete the backup file.', 'csrf_token' => $new_csrf_token]);
                        }
                    }
                }else{
                    echo json_encode(['status' => 'false', 'title' => 'Request Failed', 'message' => 'Invalid request' , 'csrf_token' => $new_csrf_token]);
            ...
            if($action == "system-settings-backup-download"){
                if($global_user_login == true){
                    if (!empty($pp_demo_mode)) {
                        echo json_encode(['status' => "false", 'title' => 'Demo Restriction', 'message' => 'This feature is disabled in the demo version.', 'csrf_token' => $new_csrf_token]);
                    }else{
