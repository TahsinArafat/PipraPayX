...
function restoreDatabasePDO($file)
{
    $pdo = connectDatabase();
    $pdo->exec("SET NAMES utf8mb4");
...
function pp_downloadReceiptPDF($data = [])
{
    ...
    if ($logo !== '') {
        ...
        $isSupported = $imageInfo && in_array($imageInfo[2], [IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_GIF], true);
        if ($isSupported) {
    ...
    $pdf->SetXY(50, 12);
    $pdf->Cell(0, 8, $brand['name'] ?? 'N/A', 0, 1);

    $pdf->SetFont('Arial', '', 10);
    $pdf->SetX(50);
    $brandCity = $brand['address']['city'] ?? '';
    $brandCountry = $brand['address']['country'] ?? '';
    $brandLocation = trim($brandCity . ($brandCity && $brandCountry ? ', ' : '') . $brandCountry);
    $pdf->Cell(0, 6, $brandLocation !== '' ? $brandLocation : 'N/A', 0, 1);
    ...
    $pdf->Ln(2);
    $pdf->SetFont('Arial', '', 11);
    $pdf->Cell(0, 6, 'Local Net Amount: ' . money_round($tx['local_net_amount'] ?? 0, 2) . ' ' . ($tx['local_currency'] ?? ''), 0, 1, 'C');
    ...
    infoRow($pdf, 'Transaction Ref', $tx['ref'] ?? 'N/A');
    infoRow($pdf, 'Payment Method', $tx['payment_method'] ?? 'N/A');
    $brandTimezone = $brand['locale']['timezone'] ?? 'Asia/Dhaka';
    if ($brandTimezone === '--' || $brandTimezone === '') {
        $brandTimezone = 'Asia/Dhaka';
    }
    infoRow($pdf, 'Created Date', isset($tx['created_date']) ? convertUTCtoUserTZ($tx['created_date'], $brandTimezone, "M d, Y h:i A") : 'N/A');

    $pdf->Ln(3);
    sectionTitle($pdf, 'Customer Details');
    $customer = $tx['customer'] ?? [];
    infoRow($pdf, 'Name', $customer['name'] ?? 'N/A');
    infoRow($pdf, 'Email', $customer['email'] ?? 'N/A');
    infoRow($pdf, 'Mobile', $customer['mobile'] ?? 'N/A');

    $pdf->Ln(3);
    sectionTitle($pdf, 'Payment Breakdown');
    infoRow($pdf, 'Amount', money_round($tx['amount'] ?? 0, 2) . ' ' . ($tx['currency'] ?? ''));
    infoRow($pdf, 'Discount', money_round($tx['discount_amount'] ?? 0, 2) . ' ' . ($tx['currency'] ?? ''));
    infoRow($pdf, 'Processing Fee', money_round($tx['processing_fee'] ?? 0, 2) . ' ' . ($tx['currency'] ?? ''));
    ...
    $pdf->Output('D', 'Receipt-' . ($tx['ref'] ?? 'payment') . '.pdf');
    exit;
}
...
/*
 * Notification & Instant Alert Channels (ported from samsusiyam/PipraPay)
 * These back the System Settings -> Notification page and the
 * system-settings-notification-* adapter actions. Credentials default to
 * the get_env() option store, but can be overridden per-call.
 */
